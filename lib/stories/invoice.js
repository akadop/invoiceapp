...
export const createInvoice = () => async (dispatch, getState, client) => {
  ...
  try {
    const { items, customer, payment = false } = values
    if (!items || !customer || !payment || !scheduleDate || !storeName) {
...
export const searchInvoice = () => (dispatch, getState) => {
  ...
  if (!values) {
    return dispatch(filterInvoice({ name: '' }))
  }

  const { name } = values

  dispatch(filterInvoice({ name: !values ? '' : name }))
}
